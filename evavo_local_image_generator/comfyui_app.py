...
<section class="native card"><div><strong>Native workstation controls</strong><div class="sub">Open the full node editor or the independent backup app.</div></div><div class="actions" style="margin-top:0"><button id="openNative">Open full ComfyUI</button><button id="openBackup">Open backup app</button></div></section>
...
$("openBackup").addEventListener("click",async()=>{const b=$("openBackup");b.disabled=true;status("Opening backup…");try{const data=unpack(await call("open_evavo_comfyui_app",{port:8770,wait_seconds:30}));status(data.ok?"Backup app ready":"Backup failed",data.ok?"ok":"bad");log(data)}catch(e){status("Backup failed","bad");log({message:e.message})}finally{b.disabled=false}});
