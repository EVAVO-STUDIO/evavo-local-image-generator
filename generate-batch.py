#!/usr/bin/env python3
"""Concurrent batch image queueing for EVAVO mock or native ComfyUI."""
...
async def queue_generation(prompt: str, project_name: str, endpoint: str, semaphore: asyncio.Semaphore, timeout: float) -> Dict[str, Any]:
...
async def batch_generate(prompts: List[str], project_name: str, endpoint: str, concurrency: int, timeout: float) -> List[Dict[str, Any]]:
    semaphore = asyncio.Semaphore(concurrency)
    return await asyncio.gather(*(queue_generation(prompt, project_name, endpoint, semaphore, timeout) for prompt in prompts))
...
def display_results(results: List[Dict[str, Any]]) -> None:
    ...
    print("=" * 112)
    print(f"{'#':<4} {'Status':<10} {'Task ID':<38} {'Project':<18} {'Backend':<16} {'Prompt':<24}")
    print("-" * 112)
    for index, result in enumerate(results, 1):
        ...
        print(f"{index:<4} {status:<10} {task_id:<38} {project:<18} {backend:<16} {prompt:<24}")
