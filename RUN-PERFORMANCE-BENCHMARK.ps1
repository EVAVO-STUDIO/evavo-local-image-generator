...
Write-Host "EVAVO GPU PERFORMANCE BENCHMARK" -ForegroundColor Cyan
...
Write-Host "Telemetry interval: $TelemetryInterval s"
Write-Host ""

& $Python .\test_frozen_quality_environment.py
if ($LASTEXITCODE -ne 0) {
    throw "Frozen-recipe regressions failed. Performance benchmark was not started."
}
...
if ($RequireGpuTelemetry) {
    $args += "--require-gpu-telemetry"
}
...
# Capture the runtime separately from the timing loop so model hashing and Git
# inspection cannot contaminate render timings.
...
Write-Host ""
Write-Host "Measured performance evidence completed." -ForegroundColor Green
Write-Host "Performance: $($performance.FullName)"
Write-Host "Runtime evidence: $runtimeEvidence"
Write-Host "This command does not change image profiles, sampler defaults or batch concurrency."
